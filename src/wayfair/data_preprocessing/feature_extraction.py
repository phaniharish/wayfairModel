...
from wayfair.data_preprocessing.data_dao import OrderDAO, \
    InventoryDAO, ZipcodeDAO, PaymentsDAO
...
class FeatureExtractor:
    ...
    @staticmethod
    def add_payment_status(orders_data: pd.DataFrame,
                           payments_data: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        """
        assert ("order_id" in payments_data.columns)\
               and ("payment_status" in payments_data.columns)
        assert "order_id" in orders_data.columns

        orders_data_with_payment = pd.merge_asof(orders_data, payments_data,
                                                 by='order_id', on=config.default_time_column)

        assert orders_data_with_payment.payment_status.isna().sum() == 0
        return orders_data_with_payment

    @staticmethod
    def add_inventory_info(orders_data: pd.DataFrame,
        ...
        :return:
        """
        assert ("sku_code" in inventory_data.columns) \
               and ("inventory" in inventory_data.columns)
        assert "sku_code" in orders_data.columns

        inventory_data.set_index('sku_code', inplace=True)  # data already sorted by time
        ...
        for i, row in orders_data.iterrows():
            ...
            inventory_info_order_time = inventory_info[(inventory_info[config.default_time_column] <=
                                                        row[config.default_time_column])]
            inventory_info_order_time = inventory_info_order_time.sort_values(config.default_time_column)
            if len(inventory_info_order_time.shape) > 0 and inventory_info_order_time.shape[0] > 0:
    ...
    @staticmethod
    def add_zipcode_data(orders_data: pd.DataFrame,
                         zipcode_data: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def add_derivative_columns(features: pd.DataFrame):
        ...
        :return:
        """
        features['zip_code_available'] = features['timestamp'] > \
    ...
    def store_features(feature_data):
        ...
        :return:
        """
        feature_data[['order_id', 'order_hour_of_day',
                      'inventory', 'payment_status', 'zip_code_available']].to_csv(
            config.features_data_path, index=False
        )
    ...
    def extract_features(self) -> pd.DataFrame:
        ...
        :return:
        """
        orders_with_payments = self.add_payment_status(orders_data=self.orders_data,
                                                       payments_data=self.payments_data)

        inventory_merged_features = self.add_inventory_info(orders_data=orders_with_payments,
                                                            inventory_data=self.inventory_data)
        zipcode_merged_features = self.add_zipcode_data(orders_data=inventory_merged_features,
                                                        zipcode_data=self.zipcode_data)
        feature_data = self.add_derivative_columns(zipcode_merged_features)
        required_features = feature_data[['order_id',
                                          'order_hour_of_day', 'inventory',
                                          'payment_status', 'zip_code_available']]
        required_features.set_index('order_id', inplace=True)
        required_features.to_csv(
